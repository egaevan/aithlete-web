...
export const scheduleService = {
  ...
  async getTodaySchedule(): Promise<Schedule[]> {
    ...
    const response = await apiClient.get<ApiResponse<Schedule[]>>('/schedules/today', { date: today })
    return response.data
  },
  ...
  async toggleComplete(id: string, completed: boolean): Promise<Schedule> {
    const response = await apiClient.patch<ApiResponse<Schedule>>(`/schedules/${id}/toggle`, { completed })
    return response.data
  },
}
